# caps

Practicing on Code Academy Parcel Service using Socket.io connection with 2 clients and a server


## Pull Request : 

* [PR:1](https://github.com/Mohammed-Awadallah/caps/pull/1)
* [PR:2](https://github.com/Mohammed-Awadallah/caps/pull/5)


 ## UML 
  ### events
  ![UML](https://cdn.discordapp.com/attachments/933696699040407562/953920912577658920/mohammed-UML.png)

  ### socket.io
  ![UML](https://i.ibb.co/jJ2nJQG/uml.png)


  ## Output : 
  ### socket .io
  ![socket.io](https://i.ibb.co/fM4TnFj/terminal.png)
